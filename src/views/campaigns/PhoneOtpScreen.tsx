...
function PhoneOtpScreen({ handleSuccess }: Props) {
      ...
      if (packageResponse?.package_type === 0) {
        ...
        if (packageResponse?.network_type) {
          ...
          if (networkType === 1 || networkType === 2) {
            subPackage.mutate({ ...subPackageObj, package: 1 });
          } else if (networkType === 3) {
            subPackage.mutate({ ...subPackageObj, package: 3 });
          }
        } else {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Stack spacing={7} align={"center"} maxW={"350px"}>
        <FormControl
        ...
        {(otpRecord?.network_type === 1 || otpRecord?.network_type === 2) && (
          <Package {...packagesPrepaid[0]} />
        )}
        {otpRecord?.network_type === 3 && <Package {...packagesPostpaid[0]} />}
...
export default memo(PhoneOtpScreen);
